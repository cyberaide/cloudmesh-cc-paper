...
in Figure~\ref{}, where we explain how easy it is to set up a workflow
with the Python API.
...
\begin{minted}[breaklines]{bash}
    ...
    w.add_dependencies(dependency="start,fetch-data,compute,analyze")
    w.run()
\end{minted}
\end{figure}
...
Certainly one can also directly interact with the REST service API for this service.
For ease of use we have exposed the interface thwough an OpenAPI specification that is availeable to the user as shown in Figure~\ref{fig:openapi}.
...
To also allow programming against the REST service in Python a Python API similar to that pf the commanindline mode is avalable. Figure~\ref{fig:code-workflow-rest-commandline} showcases this API.
