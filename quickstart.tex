...
class Workflow(...):
    ...
    add_dependency(source, destination)
        """Add a job dependency to the workflow (and the graph)."""
    def add_job(filename=None, name=None, user=None,
                host=None, label=None,
                label_format=None, kind='local',
                status='ready', progress=0,
                script=None, exec=None, pid=None,
                venv=None, **kwargs)

        """Add a job to the workflow where the parameters are
    ...
    def get_predecessors(name)
        """Return the predecessors of jobs where the predecessor status is not 100"""
    def job(name):
    ...
    property yaml
        """Return the yaml file of the workflow with status."""
    property json
        """Return the json file of the workflow with status."""
